...
For this example we assume a scenario with two clusters: kind-kind and production.
...
- **apps/kind-kind/** dir contains the kind-kind values
...
In **apps/kind-kind/** dir we have a Kustomize patch with the kind-kind specific values:
...
## Bootstrap kind-kind and production
...
In **clusters/kind-kind/** dir we have the Kustomization definitions:
...
Note that with `path: ./apps/kind-kind` we configure Flux to sync the kind-kind Kustomize overlay and 
...
Verify that your kind-kind cluster satisfies the prerequisites with:

```sh
flux check --pre
```

Set the kubectl context to your kind-kind cluster and bootstrap Flux:
...
The bootstrap command commits the manifests for the Flux components in `clusters/kind-kind/flux-system` dir
and creates a deploy key with read-only access on GitHub, so it can pull changes inside the cluster.

Watch for the Helm releases being install on kind-kind:
...
$ curl -H "Host: podinfo.kind-kind" http://localhost:8080
{
...
```sh
kubectl --context kind-kind -n redis get secrets
...
Copy the sync manifests from kind-kind:

```sh
cp clusters/kind-kind/infrastructure.yaml clusters/dev
cp clusters/kind-kind/apps.yaml clusters/dev
```
...
* the [e2e](./.github/workflows/e2e.yaml) workflow starts a Kubernetes cluster in CI and tests the kind-kind setup by running Flux in Kubernetes Kind
